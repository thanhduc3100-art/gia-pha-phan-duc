 ...
 * Tác giả gốc: TS. Hồ Ngọc Đức
 * Bản đầy đủ - Đã khắc phục lỗi tính toán Epoch (1900)
 */
...
function jdToDate(jd) {
    ...
    if (jd > 2299160) {
        ...
        d = b - INT((1461 * c) / 4);
        e = INT((5 * d + 2) / 153);
        day = d - INT((153 * e + 2) / 5) + 1;
        month = e + 3 - 12 * INT(e / 10);
        year = c - 4800 + INT(e / 10);
    }
...
function getSunLongitude(dayNumber, timeZone) {
    var T = (dayNumber - 2451545.0 - timeZone / 24) / 36525.0;
    var T2 = T * T;
    var dr = Math.PI / 180;
...
function getLunarDate(dd, mm, yyyy, timeZone) {
    ...
    var a11 = getLunarMonth11(yyyy - 1, timeZone);
    var b11 = getLunarMonth11(yyyy, timeZone);
    var k = INT((jd - a11) / 29.530588853);
    var isLeap = 0;
    var leapMonth = 0;
    if (INT((b11 - a11) / 29) == 13) {
        leapMonth = getLeapMonthOffset(a11, timeZone);
    }
    var nm = getNewMoonDay(k + INT((a11 - 2415021.076998695) / 29.530588853) + 0.5, timeZone);
    if (nm > jd) {
        k--;
        nm = getNewMoonDay(k + INT((a11 - 2415021.076998695) / 29.530588853) + 0.5, timeZone);
    }
    var month = k - 1;
    if (leapMonth > 0 && k >= leapMonth) {
        month = k - 2;
        if (k == leapMonth) isLeap = 1;
    }
    if (month < 1) month += 12;
    return [jd - nm + 1, month, yyyy + (month < 11 ? 0 : -1), isLeap];
}
...
function getSolarDate(lunarDay, lunarMonth, lunarYear, lunarLeap, timeZone) {
    ...
    var month = lunarMonth < 11 ? lunarMonth + 12 : lunarMonth;
    var k = month - 11;
    if (INT((b11 - a11) / 29) == 13) {
